...
def _int(value: str | None) -> int | None:
    """Число из строки запроса.

    Пустое значение это «не задано», а не ноль: `count=` без значения провайдер
    шлёт при сбросе постраничности, и трактовка его нулём отдала бы пустую
    страницу при ненулевом счётчике.
    """
