...
class HealthController(Controller):
    ...
    @get(opt={PUBLIC: True})
    async def health(
        self, db_session: NamedDependency[AsyncSession], cache: NamedDependency[Cache]
    ) -> dict:
    ...
    @get("/live", opt={PUBLIC: True})
    async def live(self) -> dict:
        """Жив ли процесс.

        Отдельно от готовности, и разница существенная. Готовность отвечает
        «могу обслуживать» и краснеет, когда лежит база; живость отвечает
        «процесс не завис». Перезапускать контейнер из-за недоступной базы
        неверно — он поднимется в то же состояние, — поэтому у перезапуска
        должна быть своя проверка, ничего не спрашивающая у соседей.
        """
        return {"status": "ok"}
