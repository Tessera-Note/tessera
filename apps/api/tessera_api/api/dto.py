...
class WorkspaceView(msgspec.Struct):
    ...
    #: Разрешены ли личные пространства. Здесь, а не в настройках: настройки
    #: читает только администратор, а кнопка «завести своё» нужна участнику.
    allowPersonalSpaces: bool = False  # noqa: N815 — имя поля из v1
    #: Включён ли помощник. Здесь по той же причине: поле обращения к нему
    #: стоит на главной, у всех.
    aiChatEnabled: bool = False  # noqa: N815 — рядом с остальными признаками
    aiGenerativeEnabled: bool = True  # noqa: N815 — рядом с остальными признаками
    #: Вправе ли обычный участник заводить шаблоны. Здесь по той же причине:
    #: кнопка «Новый шаблон» стоит на экране шаблонов, у всех, а сам признак
    #: лежит в настройках, которые читает только администратор.
    allowMemberTemplates: bool = False  # noqa: N815 — имя поля из v1
    #: С чего открывать страницу тому, кто своего выбора не делал. Здесь по той
    #: же причине: решение принимается на каждом открытии страницы, у всех.
...
class SpaceView(msgspec.Struct):
    ...
    logo: str | None = None
    #: Запрещена ли публикация страниц этого пространства наружу. Признак
    #: читает служба ссылок; здесь он нужен экрану настроек, где его меняют.
    disablePublicSharing: bool = False  # noqa: N815 — имя поля из v1
    #: Вправе ли читатель комментировать. Умолчание «нет», как в v1: право
    #: читать не даёт права писать в пространство, пусть и в обсуждение.
    allowViewerComments: bool = False  # noqa: N815 — имя поля из v1
