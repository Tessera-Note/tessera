...
from tessera_api.api.ai_chat import AiChatController
...
from tessera_api.api.version import VersionController
...
def create_app(settings: Settings | None = None) -> Litestar:
    ...
    return Litestar(
        ...
        # новом.
        # Предел тела запроса. Своё умолчание Litestar держит на десяти
        # мегабайтах, и оно молча отменяет объявленный предел ввоза: архив в
        # семьдесят мегабайт отвергался с 413 раньше, чем приложение успевало
        # его прочитать, а человеку показывалось «что-то пошло не так».
        #
        # Берётся больший из двух объявленных пределов: ввоз архива крупнее
        # обычной загрузки, а тело многочастного запроса ещё немного крупнее
        # самого файла — на обвязку.
        request_max_body_size=max(
            resolved.file_import_size_limit, resolved.file_upload_size_limit
        )
        + 10 * 1024 * 1024,
        guards=[jwt_guard],
