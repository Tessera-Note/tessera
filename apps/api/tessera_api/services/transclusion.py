    ...
    async def _sync_sources(self, page: Page) -> dict[str, int]:
        ...
        for node_id, snapshot in desired.items():
            ...
            if previous is None:
                ...
                #
                # Побеждает написавший последним: содержимое и отметка времени
                # переписываются. Проверкой покрыто то, что запись не срывается;
                # сама ветвь конфликта требует двух пишущих одновременно и в
                # проверке одной транзакцией недостижима.
