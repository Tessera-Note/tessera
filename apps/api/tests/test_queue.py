...
class TestHandlerInventory:
    ...
    #: Имена, у которых обработчика нет и это осознанно. Оба перенесены из
    #: перечня v1 вместе с остальными ради сверки двух версий по одному
    #: словарю, а сама работа в v2 идёт не заданием: вложения удалённой
    #: страницы снимает `AttachmentService.delete_page_attachments` в том же
    #: обращении, обратные ссылки пересчитывает `BacklinkService.rebuild` при
    #: сохранении. Постановки этих заданий в коде нет: проверка ниже это
    #: подтверждает.
