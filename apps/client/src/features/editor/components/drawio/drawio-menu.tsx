...
export function DrawioMenu({ editor }: EditorMenuProps) {
  ...
  const handleOpen = useCallback(async () => {
    ...
    try {
      ...
      // Ответ проверяется до чтения тела. Раньше тело ошибки скармливалось
      // загрузчику, а `finally` все равно открывал редактор: человек видел
      // пустой холст и решал, что диаграмма стерлась. Сохранив такой холст,
      // он терял ее по-настоящему.
      if (!request.ok) {
        notifications.show({
          color: "red",
          message:
            ...
            <div style={{ height: "100vh" }}>
              <DrawIoEmbed
                ...
                onExport={(data: EventExport) => {
                  saveData(data.data).catch(() => notifySaveFailed());
                }}
              />
            </div>
          </Modal.Body>
