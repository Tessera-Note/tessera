...
export default function PdfView(props: NodeViewProps) {
  ...
  /**
   * Статус читается напрямую, а не по подстроке в теле ответа внутри iframe.
   * Разбор `"statusCode":404` в тексте не находил 403, и вместо сообщения
   * пользователь видел сырой JSON ошибки в рамке документа. Заодно это
   * единственный способ отличить отсутствующий файл от закрытого правами.
   */
  useEffect(() => {
    if (!safeSrc) return;

    let cancelled = false;
    fetch(safeSrc, { method: "GET", credentials: "include", cache: "no-store" })
      .then((response) => {
