...
import { pinOffsetWatcher } from "@tessera/editor-ext";
...
export function BaseEmbedView({ node, editor, deleteNode }: NodeViewProps) {
  ...
  if (pendingKey) {
    ...
    // Сервер эти случаи различает: `getBase` отдает 404, когда страницы базы
    // нет или она удалена, а проверка доступа отдает 403. Один текст на оба
    // отправлял человека выдавать права базе, которой не существует.
    const status = getApiErrorStatus(error);

    if (status === 404) {
      // Мертвая ссылка. Узел остается в содержимом как есть и правке не
      // мешает, но выглядит явной отбивкой, а не пустым местом: человек
      // должен видеть, что здесь была база, и удалить блок сам.
      content = (
        <Box
          p="md"
          style={{
        ...
        >
          <Text c="dimmed">
            {t("This base no longer exists. It may have been deleted.")}
          </Text>
        </Box>
      );
    } else if (status === 403) {
      content = (
        <Box p="md" bg="gray.0" style={{ borderRadius: 8 }}>
          <Text c="dimmed">{t("You don't have access to this base.")}</Text>
        </Box>
      );
    } else {
      content = (
        <Box p="md" bg="gray.0" style={{ borderRadius: 8 }}>
          <Text c="dimmed">{t("Failed to load this base.")}</Text>
        </Box>
      );
    }
  } else {
