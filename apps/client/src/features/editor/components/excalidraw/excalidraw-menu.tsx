...
export function ExcalidrawMenu({ editor }: EditorMenuProps) {
  ...
  const handleOpen = useCallback(async () => {
    ...
    try {
      ...
      // Ответ проверяется до чтения тела: раньше тело ошибки уходило в
      // загрузчик, а `finally` все равно открывал редактор с пустым холстом,
      // и сохранение такого холста теряло диаграмму по-настоящему.
      if (!request.ok) {
        notifications.show({
          color: "red",
          message:
            request.status === 404
              ? t("This file no longer exists. It may have been deleted.")
              : request.status === 403
                ? t("You don't have access to this file.")
                : t("Failed to load this file."),
        });
        setIsLoading(false);
        return;
      }
      ...
      notifications.show({
        color: "red",
        message: t("Failed to load this file."),
      });
      setIsLoading(false);
      return;
    }
  ...
  /**
   * Отказ сохранения обязан быть виден. Раньше он гасился пустым `catch`:
   * нарисованное не сохранялось, и человек об этом не узнавал.
   */
  const notifySaveFailed = useCallback(() => {
    notifications.show({
      color: "red",
      message: t("Failed to save the diagram. Your changes were not saved."),
    });
  }, [t]);
  ...
  const handleSaveAndExit = useCallback(async () => {
    try {
      ...
      // Окно остается открытым, чтобы нарисованное можно было сохранить
      // повторно или выгрузить, но молчать об отказе нельзя.
      notifySaveFailed();
    }
  }, [saveData, close, notifySaveFailed]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (isDirtyRef.current && !isSavingRef.current) {
        saveData().catch(() => notifySaveFailed());
      }
    }, 60_000);
