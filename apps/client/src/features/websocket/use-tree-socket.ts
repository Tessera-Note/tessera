...
export const useTreeSocket = () => {
  ...
  const [, setTreeRefreshToken] = useAtom(treeRefreshTokenAtom);
    ...
    socket?.on("message", (event: WebSocketEvent) => {
      switch (event.operation) {
        ...
        case "refetchRootTreeNodeEvent":
          ...
          // Очистка обязана сопровождаться явным требованием заполнить
          // заново. Без него дерево оставалось пустым каждый раз, когда
          // перезапрос возвращал те же данные и ссылка на них не менялась.
