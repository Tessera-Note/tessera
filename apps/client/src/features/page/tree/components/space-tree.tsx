...
export default function SpaceTree({ spaceId, readOnly }: SpaceTreeProps) {
  ...
  const treeRefreshToken = useAtomValue(treeRefreshTokenAtom);
  ...
  useEffect(() => {
    ...
    // `treeRefreshToken` в зависимостях намеренно: он меняется при каждом
    // требовании перезаполнить дерево, даже когда ответ сервера идентичен и
    // ссылка на `pagesData` осталась прежней из-за структурного разделения.
    // Без него очистка узлов пространства не восстанавливалась, и навигация
    // пропадала до перезагрузки страницы.
  }, [pagesData, hasNextPage, spaceId, treeRefreshToken]);
