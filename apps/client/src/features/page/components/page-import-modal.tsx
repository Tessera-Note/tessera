...
function ImportFormatSelection({ spaceId, onClose }: ImportFormatSelection) {
  ...
  const handleFileUpload = async (selectedFiles: File[]) => {
    ...
    const pages: IPage[] = [];
    let pageCount = 0;
    // Причины отказа собираются, чтобы показать их пользователю. Сервер
    // объясняет, почему файл не принят (например, в PDF нет текстового слоя),
    // и общий текст «попробуйте снова» вместо этого объяснения вводит
    // в заблуждение: повтор такой файл не исправит.
