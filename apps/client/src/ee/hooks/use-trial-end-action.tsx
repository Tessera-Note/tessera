  ...
  useEffect(() => {
    if (isCloud() && trialDaysLeft === 0) {
      if (!pathname.startsWith("/settings")) {
        notifications.show({
          // Устойчивый идентификатор: после дополнения списка зависимостей
          // эффект отрабатывает не один раз за жизнь приложения, а при каждом
          // уходе со страницы настроек. Без идентификатора уведомления
    ...
    // читаются trialDaysLeft, pathname и isAdmin: на первом отрисовывании
    // рабочее пространство еще пусто, эффект отрабатывает вхолостую и больше не
    // перезапускается, то есть уведомление о конце пробного периода могло не
    // показаться ни разу.
