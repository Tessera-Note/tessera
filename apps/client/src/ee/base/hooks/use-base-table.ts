...
export function buildLayoutConfigPatch(table: Table<IBaseRow>): ViewConfigPatch {
  ...
  // Первичное свойство в скрытые не сохраняется: иначе конфигурация переживет
  // перезагрузку и снова разведет шапку с телом.
  const primaryIds = new Set(
    table
      .getAllColumns()
      .filter(
        (column) =>
          (column.columnDef.meta as { property?: IBaseProperty } | undefined)
  ...
  const hiddenPropertyIds = Object.entries(state.columnVisibility)
    .filter(
      ([id, visible]) =>
        id !== "__row_number" && !visible && !primaryIds.has(id),
    )
    .map(([id]) => id);
