...
> = {
  /**
   * Первичное свойство базы.
   *
   * Сервер создает его с типом title (createBase, convertPageToBase), а в
   * реестре дескриптора не было: getDescriptor возвращал undefined, GridCell
   * выходил через `if (!CellComponent) return null`, и ячейка не давала узла.
   * Шапка колонку рисовала, тело нет, поэтому все ячейки правее съезжали на
   * ширину закрепленной колонки.
   *
   * Отрисовка та же, что у text. Первичность и кнопка разворота строки
   * обслуживаются через property.isPrimary в GridCell, дублировать их здесь
   * не нужно. В PROPERTY_PICKER_ORDER тип не входит: пользователь не создает
   * и не выбирает его.
   */
  title: {
    type: "title",
