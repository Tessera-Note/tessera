...
function hasDetails(entry: IAuditLog): boolean {
  return !!(entry.changes?.fields?.length || entry.metadata);
}
...
function ChangedFields({ changes }: { changes: IAuditLog["changes"] }) {
  ...
  return (
    <Box>
      ...
      <Group gap={6} wrap="wrap">
        {fields.map((field) => (
          <Badge key={field} size="xs" variant="light" tt="none">
            {field}
          </Badge>
        ))}
      </Group>
                        ...
                        <Collapse expanded={isExpanded}>
                          ...
                          >
                            <Group gap="xl" align="flex-start">
                              {entry.changes && (
                                <ChangedFields changes={entry.changes} />
                              )}
                              {entry.metadata && (
