...
export default function AuditLogs() {
  ...
  const { data: retentionData } = useAuditRetentionQuery(isAdmin);
  ...
  const { data, isLoading } = useAuditLogsQuery(params, isAdmin);

  // Право то же, что на сервере: `Manage/Audit` есть у владельца и
  // администратора, и пункт в боковой панели показан администратору. Проверка
  // по владельцу приводила к тому, что администратор переходил по видимой
  // ссылке и получал пустой экран. Отказ показывается заглушкой, а не пустотой:
  // белая страница неотличима от поломки.
  if (!isAdmin) {
    return <Error404 />;
  }
