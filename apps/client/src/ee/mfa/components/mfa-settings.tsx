        ...
        <ResponsiveSettingsControl>
          {!isMfaEnabled ? (
            ...
            <Group gap="sm" wrap="nowrap">
              {/*
                Малый остаток резервных кодов виден прямо на кнопке: узнать
                об этом в момент потери приложения уже поздно, перевыпуск
                возможен только пока пользователь может войти.
              */}
              <Tooltip
                label={t(
                  "Few backup codes left. Regenerate them while you can still sign in.",
                )}
                disabled={!mfaStatus?.backupCodesLow}
                withArrow
              >
                <Button
                  variant="default"
                  size="sm"
                  color={mfaStatus?.backupCodesLow ? "orange" : undefined}
                  leftSection={
                    mfaStatus?.backupCodesLow ? (
