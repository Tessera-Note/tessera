...
/**
 * Сброс второго фактора у другого пользователя. Доступен администратору
 * рабочего пространства, право проверяет сервер.
 */
export async function resetUserMfa(data: {
  userId: string;
}): Promise<{ success: boolean }> {
  const req = await api.post<{ success: boolean }>("/mfa/reset", data);
  return req.data;
}
