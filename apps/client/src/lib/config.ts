...
import { sanitizeUrl } from "@tessera/editor-ext";
...
export function getDrawioUrl() {
  return getConfigValue("DRAWIO_URL", "http://localhost:8081");
}

/**
 * Адрес tessera-hub. Оттуда берутся документация, выпуски, лицензия и
 * страница поддержки, наружу за ними приложение не ходит.
 */
export function getHubUrl(): string {
  return getConfigValue("HUB_URL", "http://localhost:4000").replace(/\/$/, "");
}

export function getDocsUrl(slug?: string): string {
  return slug ? `${getHubUrl()}/docs/${slug}` : `${getHubUrl()}/docs`;
}

export function getReleasesUrl(): string {
  return `${getHubUrl()}/releases`;
}

export function getLicenseUrl(): string {
  return `${getHubUrl()}/license`;
}

export function getSupportUrl(): string {
  return `${getHubUrl()}/support`;
}
