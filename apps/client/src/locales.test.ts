...
describe("словари i18next", () => {
  ...
  /**
   * Строки, объявленные полями `message`, `label`, `title` и `description`,
   * до `t(...)` не доходят: схема проверки живет на уровне модуля, где хука
   * перевода нет, а справочник отдает строку, которую переводит уже
   * потребитель. Ключом служит сама фраза, поэтому она обязана быть в
   * источнике, иначе перевода не будет ни на одном языке. Так и было у меню
   * вставки и у меток журнала аудита.
   *
   * Имя собственное, название формата или языка заводится в источнике
   * тождественно. Это дешевле списка исключений и не дает пропустить
   * настоящую строку, спрятавшуюся среди имен.
   */
  it("строка справочника заведена в источнике", () => {
    const srcDir = path.resolve(__dirname);
      ...
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          for (const match of text.matchAll(literal)) {
            ...
            // Пустая подпись это не строка интерфейса, а отсутствие подписи.
