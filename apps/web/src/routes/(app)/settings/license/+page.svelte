<script lang="ts">
   ...
   *
   * Перечень из v1: экземпляр разворачивается у себя и наружу за подтверждением
   * не ходит, поэтому здесь не проверка ключа, а перечисление того, что уже
   * работает.
   *
   * Из перечня v1 не повторяется одна строка — сменный поисковый движок: в v1
   * его задаёт `SEARCH_DRIVER`, во второй версии поиск один и заменить его
   * нечем. Строка, обещающая несуществующее, хуже её отсутствия.
   */
  const features = [
