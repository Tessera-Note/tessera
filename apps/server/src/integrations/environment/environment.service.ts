...
export class EnvironmentService {
  ...
  getAppName(): string {
    return this.configService.get<string>('APP_NAME', 'Tessera');
  }
  ...
  getMailFromName(): string {
    return this.configService.get<string>('MAIL_FROM_NAME', 'Tessera');
  }
  ...
  /**
   * Адрес tessera-hub внутри сети compose. Сюда сервер ходит за версией и
   * сюда же отправляет телеметрию.
   */
  getHubInternalUrl(): string {
    return this.configService.get<string>(
      'HUB_INTERNAL_URL',
      'http://tessera-hub:4000',
    );
  }

  /**
   * Адрес tessera-hub для браузера. Отличается от внутреннего: имя сервиса
   * compose снаружи не резолвится.
   */
  getHubUrl(): string {
    return this.configService.get<string>('HUB_URL', 'http://localhost:4000');
  }
   ...
   *
   * Задается переменной окружения, а не настройкой в интерфейсе: если
   * провайдер сломан, до интерфейса администратор уже не дойдет. Требует
   * доступа к развертыванию, то есть к тому же уровню, где чинят провайдера.
   *
   * Ограничения по времени нет намеренно: снимает признак тот, кто его
   * поставил. Чтобы он не остался включенным навсегда незаметно, приложение
   * пишет предупреждение при старте и событие в журнал аудита.
   */
  /**
   * Учетные данные OAuth-клиента Google.
   *
   * Задаются переменными окружения, а не колонками провайдера: обратный
   * адрес регистрируется у Google один на установку и не может содержать
   * ни поддомен пространства, ни идентификатор провайдера. Клиент это уже
   * учитывает, строя адрес от `APP_URL`.
   */
  ...
  isSsoEmergencyAccessEnabled(): boolean {
    const enabled = this.configService
      .get<string>('SSO_EMERGENCY_PASSWORD_LOGIN', 'false')
      .toLowerCase();
    return enabled === 'true';
  }
