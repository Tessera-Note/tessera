 ...
 * зависимость в рантайме запрещена. Поэтому каталог свой, той же формы, что и
 * коды отказов: ключ, подстановки в двойных фигурных скобках, английский как
 * запасной вариант.
 *
 * Языки только те, что ведутся в репозитории. Для остальных девяти локалей
 * письмо приходит по-английски: сочинять перевод на языке, которого не знаешь,
 * хуже, чем оставить понятный английский. Когда Crowdin закроет словари
 * клиента, эти строки можно будет перенести оттуда.
 *
 * Выделение внутри предложения намеренно снято: разметка внутри фразы
 * заставляет разрезать ее на куски, а куски невозможно перевести на язык с
 * другим порядком слов. Название страницы и имя человека остаются
 * подстановками.
 */
...
const EN = {
  ...
  'mail.subject.permission_granted':
    '{{actor}} gave you {{access}} access to {{page}}',
  ...
  'mail.comment_mention.body':
    '{{actor}} mentioned you in a comment on {{page}}.',
  ...
  'mail.digest.body':
    'There have been {{count}} page updates since the last digest.',
  'mail.digest.edited_by': 'Edited by {{names}}',

  'mail.approval_requested.body':
    '{{actor}} submitted {{page}} in {{space}} for your approval.',
  'mail.approval_rejected.body':
    '{{actor}} sent {{page}} in {{space}} back for revision.',
  'mail.verification_expiring.body':
    'The page {{page}} in {{space}} needs to be verified again. Verification expires on {{date}}.',
  'mail.verification_expired.body':
    'Verification of {{page}} in {{space}} has expired. Verify the page again to confirm it is still correct.',
  ...
  'mail.mfa_reset.body':
    'An administrator reset two-factor authentication for your account{{scope}}. A second factor is no longer required to sign in.',
  'mail.mfa_reset.next':
...
const RU: Record<string, string> = {
  ...
  'mail.subject.permission_granted':
    '{{actor}} открыл вам доступ к «{{page}}» на {{access}}',
  ...
  'mail.comment_mention.body':
    '{{actor}} упомянул вас в комментарии на «{{page}}».',
  'mail.comment_resolved.body': '{{actor}} закрыл обсуждение на «{{page}}».',
  'mail.permission_granted.body':
    '{{actor}} открыл вам доступ к «{{page}}» на {{access}}.',
  'mail.page_update.body':
    '{{actor}} изменил «{{page}}» в пространстве {{space}}.',
  ...
  'mail.verification_expiring.body':
    'Страницу «{{page}}» в пространстве {{space}} пора перепроверить. Срок подтверждения истекает {{date}}.',
  'mail.verification_expired.body':
    'Срок подтверждения «{{page}}» в пространстве {{space}} истек. Проверьте страницу заново и подтвердите, что она по-прежнему верна.',
  ...
  'mail.mfa_reset.body':
    'Администратор сбросил двухфакторную аутентификацию для вашей учетной записи{{scope}}. Второй фактор для входа больше не нужен.',
  'mail.mfa_reset.next':
    'Настройте двухфакторную аутентификацию заново в настройках профиля.',
  'mail.mfa_reset.warning':
    'Если вы этого не просили, немедленно обратитесь к администратору.',
  'mail.invitation.body':
    'Вас пригласили в Tessera, базу знаний вашей команды.',
  'mail.invitation.note':
    'Вы получили это письмо, потому что вас пригласил кто-то из команды.',
  'mail.invitation_accepted.body':
...
const UK: Record<string, string> = {
  ...
  'mail.subject.permission_granted':
    '{{actor}} відкрив вам доступ до «{{page}}» на {{access}}',
  ...
  'mail.verification_expiring.body':
    'Сторінку «{{page}}» у просторі {{space}} час перевірити знову. Термін підтвердження спливає {{date}}.',
  'mail.verification_expired.body':
    'Термін підтвердження «{{page}}» у просторі {{space}} минув. Перевірте сторінку заново та підтвердіть, що вона й досі правильна.',
  ...
  'mail.mfa_reset.body':
    'Адміністратор скинув двофакторну автентифікацію для вашого облікового запису{{scope}}. Другий фактор для входу більше не потрібен.',
  'mail.mfa_reset.next':
    'Налаштуйте двофакторну автентифікацію заново в налаштуваннях профілю.',
  'mail.mfa_reset.warning':
  ...
  'mail.invitation.note':
    'Ви отримали цей лист, бо вас запросив хтось із команди.',
  'mail.invitation_accepted.body':
...
export const MAIL_LOCALES = Object.keys(CATALOGUE);

/**
 * Все ключи каталога.
 *
 * Английский полон по построению, он же запасной вариант, поэтому список
 * ключей берется из него. Нужен проверке полноты остальных языков: ключ,
 * заведенный не везде, дает письмо вперемешку, часть строк переведена, часть
 * нет, и заметить это иначе нечем.
 */
export const MAIL_TEXT_KEYS = Object.keys(EN);

/** Строка языка напрямую, без запасного варианта. Только для проверок. */
export function mailTextRaw(locale: string, key: string): string | undefined {
  return CATALOGUE[locale]?.[key];
}

/** Язык писем по умолчанию: тот же, что запасная локаль клиента. */
export const DEFAULT_MAIL_LOCALE = 'en-US';

export type MailTextKey = keyof typeof EN;

/**
 * Строка письма на языке получателя.
 *
 * Неизвестный язык и незаполненный ключ дают английский: письмо на понятном
 * чужом языке лучше пустоты и лучше ключа на экране.
 */
