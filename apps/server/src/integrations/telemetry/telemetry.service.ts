...
export class TelemetryService {
  // Телеметрия остается внутри сети compose: события принимает tessera-hub.
  private get endpointUrl(): string {
    const base = this.environmentService.getHubInternalUrl().replace(/\/$/, '');
  ...
  async sendTelemetry() {
    try {
      ...
      await fetch(this.endpointUrl, {
        method: 'POST',
        headers: {
          'User-Agent': 'tessera:' + data.version,
