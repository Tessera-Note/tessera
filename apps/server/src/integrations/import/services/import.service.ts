...
export class ImportService {
  ...
  async importPage(
    ...
    try {
      ...
      // Осознанное сообщение разборщика доходит до пользователя как есть.
      // Общий текст скрывал бы причину отказа: скан без текстового слоя,
      // пустой или битый файл. Неожиданные ошибки по-прежнему сворачиваются
      // в общее сообщение, чтобы наружу не утекали внутренние детали.
      if (err instanceof BadRequestException) {
        this.logger.debug(`Импорт отклонен: ${err.message}`);
        throw err;
      }
