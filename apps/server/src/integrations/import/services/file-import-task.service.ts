...
import { FileTask, InsertablePage } from '@tessera/db/types/entity.types';
...
export class FileImportTaskService {
  ...
  async processGenericImport(opts: {
    ...
    const tesseraMetadata = await readTesseraMetadata(extractDir);
    ...
    for (const absPath of allFiles) {
      ...
      const pageMetadata = tesseraMetadata?.pages[encodedPath];
    ...
    sortedFolders.forEach((folderPath) => {
      ...
      if (!pagesMap.has(mdPath) && !pagesMap.has(htmlPath)) {
        ...
        if (!matched) {
          ...
          const placeholderMetadata = tesseraMetadata?.pages[encodedMdPath];
          pagesMap.set(mdPath, {
            id: v7(),
            slugId: generateSlugId(),
    ...
    const sortSiblings = (siblings: ImportPageNode[]) => {
      if (tesseraMetadata) {
        siblings.sort((a, b) => {
          const posA =
            tesseraMetadata.pages[encodedPathsMap.get(a.filePath)]?.position;
          const posB =
            tesseraMetadata.pages[encodedPathsMap.get(b.filePath)]?.position;
          if (posA && posB) {
