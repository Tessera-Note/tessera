  ...
  async processAttachments(opts: {
    ...
    for (const aEl of $('a').toArray()) {
      ...
      if (ext === '.pdf') {
        ...
        // Порядок источников имени: явный алиас Confluence, затем настоящее
        // имя из раздела вложений страницы, и только потом имя файла
        // в архиве. Confluence Server хранит файлы под числовыми именами,
        // поэтому без второго источника вложение подписывалось бы в странице
        // как «65604», хотя в базе лежит под настоящим именем.
        const confAliasName = $a.attr('data-linked-resource-default-alias');
        const attachmentName =
          confAliasName ||
          attachmentNameByRelPath.get(relPath) ||
          path.basename(abs);
