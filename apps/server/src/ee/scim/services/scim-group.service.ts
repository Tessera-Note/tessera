...
export class ScimGroupService {
      ...
      const group = await this.groupRepo.insertGroup(
        {
          ...
          // каталогом, а не человеком. Он оставлен ради совместимости, а
          // владение выражает привязка ниже.
          isExternal: true,
          // Привязка: группу ведет SCIM. Ключ каталога необязателен по
          // RFC 7643, и его отсутствие владения не отменяет.
          directorySource: 'scim',
          directoryKey: externalId ?? null,
   ...
   *
   * Признак принадлежности это `directory_source`, а не `scim_external_id`.
   ...
   * изменение отказом с ложной причиной. `directory_source` ставится при
   * заведении всегда.
   *
   * Проверяется именно источник, а не сам факт привязки: группа, которую
   * ведет провайдер SSO, каталогу SCIM не принадлежит, и права менять и
   * удалять ее у него нет.
   *
  ...
  private assertManagedByDirectory(row: GroupRow): void {
    ...
    if (row.directorySource !== 'scim') {
      throw new ScimException(
        HttpStatus.BAD_REQUEST,
        `Group ${row.name} is not managed by SCIM`,
        'mutability',
      );
    }
  }
