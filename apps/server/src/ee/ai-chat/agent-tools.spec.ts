...
describe('Инструменты агента', () => {
  ...
  it('число шагов ограничено', () => {
    ...
    // Предел поднят вместе с подключением инструментов MCP: составная просьба
    // тратит ход на каждый вызов, и на двенадцати обрывалась на середине.
    // Верхняя граница все равно нужна, она держит расход у провайдера, время
    // ответа и петлю на неверном инструменте.
    expect(AGENT_MAX_STEPS).toBeLessThanOrEqual(32);
  });
});
