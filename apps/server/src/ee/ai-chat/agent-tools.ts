...
/**
 * Сколько ходов агент делает за одну просьбу.
 *
 * Было восемь, и этого хватало собственным пяти инструментам. С инструментами
 * MCP просьбы стали составными: чтобы перенести десять страниц, агент сначала
 * берет список пространств, потом список страниц, потом переносит каждую. На
 * восьми ходах такая просьба обрывалась на середине.
 *
 * Предел все равно нужен: он ограничивает и расход у провайдера, и время
 * ответа, и петлю, в которую агент может уйти на неверном инструменте.
 */
