...
export class AiChatService {
  ...
  async *sendMessage(
    ...
    const systemPrompt = this.buildSystemPrompt(
      ...
      //
      // Прежние реплики человека передаются потому, что у разговора есть язык.
      // Короткая просьба вроде «а можешь их все перенести» признаков языка не
      // содержит вовсе, и при украинской локали интерфейса ответ приходил
