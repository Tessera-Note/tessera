...
// Workspace
// Берется из DbInterface, а не напрямую из сгенерированных типов: там колонки
// хранения объявлены как bigint, то есть строка, а драйвер отдает число.
type WorkspacesTable = DbInterface['workspaces'];
export type Workspace = Selectable<WorkspacesTable>;
export type InsertableWorkspace = Insertable<WorkspacesTable>;
export type UpdatableWorkspace = Updateable<Omit<WorkspacesTable, 'id'>>;
