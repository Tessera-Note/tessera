...
import { DatabaseModule } from '@tessera/db/database.module';
...
// Аудит выбирается здесь, а не наложением двух глобальных провайдеров одного
// токена: иначе победитель зависел бы от порядка импортов. Без EE остается
// NoopAuditService, и вызовы аудита по всему серверу просто ничего не делают.
let auditModule: any = NoopAuditModule;
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const eeAuditModule = require('./ee/audit-ee/audit-ee.module')?.AuditEeModule;
  if (eeAuditModule) {
    auditModule = eeAuditModule;
  }
} catch {
  // EE не собран, аудит остается заглушкой.
}
