...
import { Attachment, User, Workspace } from '@tessera/db/types/entity.types';
...
export class AttachmentService {
  ...
  async uploadFile(opts: {
    ...
    try {
      ...
      // Задача ставится на любое вложение, а не только на разбираемые типы:
      // отметку «тип не поддерживается» тоже кто-то должен проставить, иначе
      // такие файлы навсегда остаются в состоянии «не обработано» и их
      // невозможно отличить от еще не дошедших до разбора. Решение о
      // поддержке типа принимает AttachmentEeService, в одном месте.
