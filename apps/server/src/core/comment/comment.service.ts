...
import { Comment, Page, User } from '@tessera/db/types/entity.types';
...
export class CommentService {
  ...
  /**
   * Отметить обсуждение решенным или снять отметку.
   *
   * Клиент вызывал `/comments/resolve` с самого начала, но маршрута не было:
   * колонки `resolved_at` и `resolved_by_id` существовали, а записывать в них
   * было нечем. Право на действие то же, что и на комментирование страницы,
   * проверяется в контроллере: решать обсуждение может любой участник
   * обсуждения, а не только автор комментария.
   */
  async resolve(
    comment: Comment,
    resolved: boolean,
