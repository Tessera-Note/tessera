...
export class AuthController {
  ...
  constructor(
    ...
    private readonly emergencyAccess: SsoEmergencyAccessService,
  ...
  async login(
    ...
    // Принуждение SSO. Аварийный доступ проверяется здесь же, до проверки
    // пароля: иначе он не покрыл бы путь второго фактора, где вход
    // завершается другим маршрутом.
    if (workspace.enforceSso && this.emergencyAccess.isEnabled()) {
      await this.emergencyAccess.assertAllowed(workspace, loginInput.email);
    } else {
