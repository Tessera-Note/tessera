...
import ChangePasswordEmail from '@tessera/transactional/emails/change-password-email';
import { ForgotPasswordDto } from '../dto/forgot-password.dto';
import ForgotPasswordEmail from '@tessera/transactional/emails/forgot-password-email';
import { UserTokenRepo } from '@tessera/db/repos/user-token/user-token.repo';
import { PasswordResetDto } from '../dto/password-reset.dto';
import { User, UserToken, Workspace } from '@tessera/db/types/entity.types';
...
export class AuthService {
  ...
  async login(loginDto: LoginDto, workspaceId: string) {
    ...
    // Актора ставим явно: перехватчик берет его из request.user, а при входе
    // пользователь еще не аутентифицирован, и событие ушло бы без автора.
