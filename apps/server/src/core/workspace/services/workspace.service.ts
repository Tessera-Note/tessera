...
import { isPageEmbeddingsTableExists } from '@tessera/db/helpers/helpers';
...
export class WorkspaceService {
  ...
  async deactivateUser(
    ...
    const revokedSessionIds = await executeTx(this.db, async (trx) => {
      ...
      return this.userSessionRepo.revokeByUserId(userId, workspaceId, trx);
    });
  ...
  async deleteUser(
    ...
    const revokedSessionIds = await executeTx(this.db, async (trx) => {
      await this.userRepo.updateUser(
        {
          name: 'Deleted user',
          email: v4() + '@deleted.tessera.com',
          avatarUrl: null,
          settings: null,
          deletedAt: new Date(),
          // Связь с каталогом снимается вместе с обезличиванием. Частичный
          // уникальный индекс на (workspace_id, scim_external_id) не знает
          // про deleted_at, и удержанный удаленной строкой идентификатор
          // ломал бы повторное заведение того же человека по SCIM.
          scimExternalId: null,
        },
        userId,
        workspaceId,
    ...
    // См. `deactivateUser`: без этого удаленный участник остается на связи.
    await this.wsService.disconnectSessions(revokedSessionIds);
