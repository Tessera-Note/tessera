...
**Некритично, но теряет молча. Минимум — предупреждение, лучше — как у Word.**
...
## Пользовательские строки мимо словаря

**Некритично, но продукт выглядит недоделанным. Работа примерно на день.**

Найдены 16 сентября 2026 словарным ревьюером и поиском текстовых узлов без
подстановки. Ни одна не появилась в правках той сессии, все четыре старше.

- **Перечень возможностей на экране лицензии.**
  `settings/license/+page.svelte:23` держит опись из семнадцати английских фраз,
  и разметка выводит их сырыми (`{feature}`, строка 89). Две из них в словарях
  есть (`Kanban`, `Templates`), остальных пятнадцати нет. Чинится вызовом `t()`
  на выводе и заведением пятнадцати фраз в двенадцати словарях.
- **Метка `Workspace ID` на том же экране**, строка 51: голый текст, тогда как
  соседние метки идут через `t()`. Других таких текстовых узлов в разметке
  экранов нет, это проверено поиском по всем `.svelte`.
- **Журнал аудита показывает код ресурса.** `settings/audit/+page.svelte:186`
  выводит `row.resourceType` как есть: на стенде в столбце видны `user`,
  `sso_provider`, `page`, `space`. Подписи событий рядом переводятся описью
  `lib/features/audit/labels.ts` — код ресурса решается так же.
- **Подписи сортировки в пакете редактора.**
  `packages/editor-ext/src/lib/table/table-readonly-sort.ts:154,157` держит
  `Sort ascending`, `Sort descending` и `Clear sort` литералами: пакет общий с
  v1 и словаря не знает. Путь передачи подписей у него уже есть —
  `setMediaErrorLabels` из `lib/stores/i18n.svelte.ts`, — и эти три ложатся тем
  же способом.
...
`trashRetentionDays` в DTO объявлен целым (`apps/api/tessera_api/api/
workspace.py:79`), и сервер отвечает отказом. Человек видит осмысленную
подсказку и непонятный отказ на сохранении.
