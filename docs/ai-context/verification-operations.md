...
Затем открыть `http://127.0.0.1:9099` — сервер поставит cookie и переведёт на
стенд. Обе команды перечислены в `allow` файла `.claude/settings.json`: без
этого классификатор режима auto их не пропускает.

Оба скрипта отказываются работать вне стенда: первый сверяет `APP_URL` с
локальными узлами, у второго адрес перехода жёстко указывает на `127.0.0.1`.
Файл `.stand-session` содержит учётные данные, он в `.gitignore` и после
осмотра удаляется.

**Канал событий на стенде отказывает по адресу `127.0.0.1`.** У стенда
`APP_URL=http://localhost:8080`, а рукопожатие сокета сверяет происхождение с
ним (`api/realtime.py`, `origin_allowed`). Со `127.0.0.1` в консоли висит
«Канал событий отказал в подключении», и это не поломка приложения: по
`http://localhost:8080` тот же стенд подключается молча. Осматривать живые
обновления надо по `localhost`.
...
**Проверки `apps/api` без `DATABASE_URL` молча пропускаются**: `uv run pytest`
покажет «734 passed, 1093 skipped» и это не прогон. Прогонять с тем же
окружением, что и стенд; полный прогон — 1830 проверок.
