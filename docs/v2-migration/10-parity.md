...
Сверка отдельная от маршрутов и сделана после них: у MCP свой перечень, и
пропуск в нём маршрутами не виден. Не хватало девятнадцати инструментов, теперь
их 65 против 64 в v1.

| v1 | v2 |
|---|---|
| `delete_base_row` | `delete_base_rows` — принимает список, а не один идентификатор |
| — | `search_web` — поиск в интернете, в v1 его нет |

## Избранное

В v1 отмечают страницы, пространства и шаблоны; в v2 отмечались только
страницы. Разница видна не по именам маршрутов, а по полю `type` в запросе, и
сверка маршрутов её не показывала. Теперь виды все три — и в HTTP, и в MCP.
