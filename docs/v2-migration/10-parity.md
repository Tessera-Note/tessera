...
Третий канал, и сверка по нему тоже отдельная. На сервере он был полностью:
комнаты, отбор получателей, события страниц, комментариев, баз и уведомлений. В
браузере не было ничего — клиента Socket.IO в `apps/web` не существовало, и ни
одно событие никуда не приходило. Живого обновления в v2 не было вовсе, хотя со
стороны сервера всё выглядело сделанным.

Теперь клиент есть: одно соединение на вкладку, подписка без выбора видов —
сервер шлёт только то, что положено этому человеку. Слушают четверо: дерево
страниц (перечитывает ветвь), обсуждение (перечитывает страницу), экран базы
(своя подписка на комнату базы) и значок непрочитанного.

Про развёртывание. Рукопожатие сверяет `Origin` с `APP_URL`, и при расхождении
сервер отвергает соединение молча — библиотека просто пробует снова. На стенде
это стоило получаса разбирательств, поэтому отказ теперь пишется в журнал
браузера отдельной строкой.
